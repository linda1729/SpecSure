...
1. 数据管理与预处理支持
   ...
   - **设计并实现数据集管理逻辑**
     - 支持 **默认数据集一键加载**（已实现并验证）
     - 预留 **用户自主上传数据接口**（已实现，待完整验证）
   - 后端已接入并支持以下预处理流程（由模型侧执行，后端负责调度）：
     - 高光谱数据降噪
     - 波段选择
     - 数据标准化
   - 后端通过接口统一管理 **数据路径、数据集配置与模型输入规范**，避免前端直接操作文件系统
   
   ------
   
   2.CNN 分类模块
   
   - **完成 CNN（HybridSN）模型的后端完整支持**：
     - 模型训练接口
     - 推理接口
     - 模型与产物统一命名与存储
   - 支持：
     ...
     - 默认模型路径一键使用
     - 模型运行日志回传
   - **SVM 模型接口已在后端预留结构**，便于后续扩展对比实验
   
   ------
   
   3.训练任务调度与异步执行（关键后端贡献）
   
   - 将模型训练由**同步阻塞**改造为：
     - **后台线程异步执行**
     - 任务状态实时记录（PID、进度、时间戳、错误信息）
   - 新增并实现：
     - 训练任务状态查询接口
     - 日志尾部解析与实时回传
   - 有效解决：
     - 前端请求超时
     - 云端 GPU 训练长任务阻塞 API 的问题
   
   ------
   
   4.分类结果与评估输出支持
   
   后端已支持并输出以下结果数据（部分前端展示仍在完善中）：
   ...
   - 模型评估指标计算：
     ...
     - Kappa 系数
     - 混淆矩阵
   - 后端已完成 **指标计算与结构化返回**，当前仅缺少独立前端展示页面
   
   ---
   
   4.后端架构重构
   
   - 统一 API 前缀为 `/api/cnn`，避免部署环境不一致导致的 404 问题
   - 精简 `main.py`，仅保留必要路由与静态挂载
   - 删除旧数据目录与冗余 services/models 代码，减少技术债务
...
<img src="./images/1.png" style="zoom:75%;" />

<img src="./images/2.png" style="zoom:75%;" />

<img src="./images/3.png" style="zoom:75%;" />
