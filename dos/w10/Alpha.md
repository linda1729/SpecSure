...
1. **完成后端框架搭建**
   ...
   - 具体的修改见：https://github.com/linda1729/SpecSure/tree/main/dos/w10/backend 中的文档。
2. **完成 mock 接口编写**
   - 在 CNN 和 SVM 同学提供完整代码前，先 mock 了一些数据来调试接口。
   - 周末的时候负责 CNN 的同学给了基础代码，我尝试去做本地联调，目前多次尝试但还未完全打通。
   - 已按目前设计完成以下 mock API：
     - `/upload_hsi`：接收文件并返回示例元数据
     - `/preprocess`：返回固定维度的处理后数据（模拟降噪 / 标准化）
     - `/classify_svm`、`/classify_cnn`：返回示例分类图（二维类别矩阵）
     - `/evaluate`：返回固定的 OA / Kappa 等指标
3. **做了一个模拟前端方便后期调试**
   - 为了能方便测试后端，我先 mock 了一个前端，给每个功能都做了一个基础界面。
   - 在这个过程中也发现了：前端最大的问题之一可能是**各类图像的呈现**（高光谱伪彩色图、光谱曲线、混淆矩阵等的可视化），后续也会和前端同学多沟通这一块。
4. **与前端同学对接**
   - 完成数据格式文档整理
   - 约定光谱曲线格式、分类图格式（二维数组）、混淆矩阵格式等
   - 一起确认前后端在 dataset_id、文件上传、模型调用链路上的参数约定
5. **组长工作**
   - 组织小组会议（包括线上/线下讨论）
...
1. **完成 CNN 模型初版（3D CNN / HybridSN）**
   - 根据论文 HybirdSN 源代码进行修改，将框架改为 PyTorch
   - 实现 patch 提取、训练脚本、评估模块
2. **完成第一次训练**
   - 使用现有高光谱数据（如 PaviaU）训练了模型，得到训练权重（PCA 和模型参数分开保存）
   - 得到初步：
...
- 在后端给的数据格式上重新训练（这个看情况，应当与原有数据格式为准，而不是 mock 的数据）
- 输出最终可部署模型
- 尝试学长多模态模型
- 代码重构与模块化
