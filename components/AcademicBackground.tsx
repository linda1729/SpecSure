...
// Augment JSX.IntrinsicElements to include React Three Fiber primitives
// This fixes TypeScript errors when the global R3F types are not properly picked up
declare global {
  namespace JSX {
    ...
    <div className="fixed inset-0 pointer-events-none -z-10 h-screen w-full transition-opacity duration-1000 ease-in-out opacity-100">
      ...
      >
        ...
        <Suspense fallback={null}>
            {/* 
               FIXED: Replaced <Environment preset="city" /> with a custom procedural environment.
               Using 'preset' triggers a download from GitHub which often fails in China.
               This manual setup creates a reflection map locally using simple emissive geometries.
            */}
            <Environment resolution={256}>
                <group rotation={[-Math.PI / 3, 0, 1]}>
                    {/* Ceiling Light for glossy reflection */}
                    <mesh position={[0, 10, 0]} scale={[10, 10, 1]} rotation={[Math.PI / 2, 0, 0]}>
                        <planeGeometry />
                        <meshBasicMaterial color="#ffffff" toneMapped={false} />
                    </mesh>
                    {/* Gold Light from side for metallic warmth */}
                    <mesh position={[10, 0, -5]} scale={[10, 10, 1]} rotation={[0, -Math.PI / 2, 0]}>
                        <planeGeometry />
                        <meshBasicMaterial color="#FFD700" toneMapped={false} />
                    </mesh>
                    {/* Fill Light */}
                    <mesh position={[-10, 5, -5]} scale={[10, 10, 1]} rotation={[0, Math.PI / 2, 0]}>
                        <planeGeometry />
                        <meshBasicMaterial color="#E7E5E4" toneMapped={false} />
                    </mesh>
                </group>
            </Environment>
