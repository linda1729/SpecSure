...
class Dot {
  ...
  update(ctx: CanvasRenderingContext2D, mouse: { x: number, y: number }, width: number, height: number, time: number, index: number, total: number, scrollY: number, theme: 'modern' | 'academic' | 'word') {
...
const ParticleSystem: React.FC<ParticleSystemProps> = ({ theme = 'modern' }) => {
  ...
  // If theme is 'word', we don't render canvas or run animation loop
  if (theme === 'word') return null;
  ...
  useEffect(() => {
    ...
    // FIX: Optimized particle count for balanced visual effect
    // High res screens get 10500, smaller screens get 2000
    let width = window.innerWidth;
    let height = window.innerHeight;
    const PARTICLE_COUNT = width > 1000 ? 3300 : 2000; 
    ...
    const render = () => {
      // Safety check if component unmounted or mode changed
      if (themeRef.current === 'word') return;
